...
public class Lerp : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            endPosition[i] = new Vector3( 
                        2f*Mathf.Sqrt(2f) * Mathf.Sin(t) *  Mathf.Sin(t) *  Mathf.Sin(t),
                        2f* (- Mathf.Cos(t) * Mathf.Cos(t) * Mathf.Cos(t) - Mathf.Cos(t) * Mathf.Cos(t) + 2 *Mathf.Cos(t)) + 3f,
                        7f + Mathf.Sin(time));
        }
    ...
    void Update()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            Renderer sphereRenderer = spheres[i].GetComponent<Renderer>();
            float hue = (float)i / numSphere; // Hue cycles through 0 to 1
            Color color = Color.HSVToRGB(Mathf.Abs(0.77f), 0.88f * Mathf.Cos(time), 2f + Mathf.Cos(time)); // Full saturation and brightness
            sphereRenderer.material.color = color;
        }
    }
}